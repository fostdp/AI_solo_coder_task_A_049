...
class TCMApplication {
    ...
    _initCharts() {
        ...
        this.charts.conductance.setOption({
            ...
            series: [{
                name: '电导', type: 'line', data: [], smooth: true, showSymbol: false,
    ...
    _updateChartsData() {
        const h = this.history[this.selectedAcupoint] || [];
        const times = h.map(d => d.time);
        const conductance = h.map(d => Number(d.skin_conductance.toFixed(2)));
        const temperature = h.map(d => Number(d.infrared_temperature.toFixed(2)));
        const emg = h.map(d => Number(d.emg_amplitude.toFixed(1)));

        this.charts.conductance.setOption({
            xAxis: { data: times },
            series: [{ name: this.selectedAcupoint + ' 电导', data: conductance }],
            title: { text: `皮肤电导 - ${this.selectedAcupoint} (μS)`, left: 10, top: 4, textStyle: { color: '#c9d1e4', fontSize: 12, fontWeight: 'normal' } }
        });
