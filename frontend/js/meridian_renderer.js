...
class MeridianRenderer {
    ...
    _drawAcupoint(ap) {
        ...
        const inSelected = !this.selectedMeridian ||
            (ap.meridian_id === this.selectedMeridian);
        if (!inSelected) {
    ...
    render() {
        ...
        if (this.options.showMeridians) {
            for (const m of this.meridians) {
                ...
                this._drawMeridian(m, active);
            }
        }

        for (const ap of this.acupoints) {
            this._drawAcupoint(ap);
        }
    ...
    _onClick(e) {
        if (this.hoveredAcupoint) {
            if (this.onAcupointClick) {
